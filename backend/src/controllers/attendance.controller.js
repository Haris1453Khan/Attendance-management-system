...
export const addAttendance = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error, Failed to add attendance." });
  }
};
...
export const viewAttendance = async (req, res) => {
  try {
    ...
    // Map attendance to include employee name
    const attendanceWithName = attendance.map((record) => ({
      _id: record._id,
      name: record.employeeId?.name || empName,
      date: record.date,
      status: record.status,
      timeIn: record.timeIn,
      timeOut: record.timeOut,
    ...
    return res.status(200).json(attendanceWithName);
  } catch (error) {
    console.error("View attendance error:", error);
    return res
      .status(500)
      .json({ message: "Server error, failed to fetch attendance." });
  }
};
...
export const deleteAttendance = async (req, res) => {
  try {
    ...
    if (!date || !name)
      return res
        .status(400)
        .json({ message: "Please fill add the required fields" });
    ...
    return res
      .status(500)
      .json({ message: "Failed while deleting attendance. Server Error." });
  }
};
